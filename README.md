...
## Mentality
  ...
  - 在实现 VIM 的光标模式上我使用的是 vue 的 v-for 实现的，这个上的更新重渲染我还暂时不知道是不是将会全部的项重新渲染来实现响应式。而这时的我有了一个新的想法，我将 viewport 这个区域作一个隐形的用于光标显示层，也就是说这个区域只有视口大小，专门用来显示光标，光标的更新（位置的定位可以使用getboundRect（）方法，当出现滚动则先滚动再定位）就只用更新这个层就行了。这就导致了我的 Block 就变成了静态的
  - VSCode 源码
    - 思想
      - 依赖注入
      - 服务
...
## Python
  ...
  - Attension
    - [Python中，使用windows的文件路径时一定要小心](https://www.jianshu.com/p/eb3d65879a90)
    - [python 不转义变量或常量](https://blog.csdn.net/nima1994/article/details/81221450)
    - [Python中os.mkdir()与os.makedirs()的区别](https://www.cnblogs.com/4c4853/p/9693539.html)
